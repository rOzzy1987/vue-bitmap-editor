...
export default {
    ...
    methods: {
        ...
        mouseMove(ev: MouseEvent) {
            this.getCanvasCoords(ev)
            if (ev.buttons == 0) this.isMouseDown = false
            if (this.isMouseDown) this.paint()
            else this.render()
        },
        mouseDown(ev: MouseEvent) {
            this.tips()
            this.getCanvasCoords(ev)
            this.penValue = ev.ctrlKey
                ? 0
                : ev.altKey
                  ? 1
                  : this.pen.color == -1
                    ? this.bitmapProp[this.bitmapCursorPos.y][this.bitmapCursorPos.x] > 0
                        ? 0
                        : 1
                    : this.pen.color
        ...
        paint() {
            ...
            this.bitmapProp = BitmapEditor.paint(this.bitmapProp, {
                pen: pen,
                loc: this.bitmapCursorPos,
            })
        ...
        fill() {
            const pen = this.penWithExplicitValue()
            this.bitmapProp = BitmapEditor.floodFill(this.bitmapProp, {
                pen: pen,
                loc: this.bitmapCursorPos,
            })
            this.lastPoint = { x: this.bitmapCursorPos.x, y: this.bitmapCursorPos.y }
        ...
        line() {
            const pen = this.penWithExplicitValue()
            this.bitmapProp = BitmapEditor.line(this.bitmapProp, {
                pen: pen,
                p1: this.bitmapCursorPos,
                p2: this.lastPoint,
            })
            this.lastPoint = { x: this.bitmapCursorPos.x, y: this.bitmapCursorPos.y }
            this.render()
        ...
        async doRender() {
            ...
            const data = BitmapRenderer.renderBitmap(this.bitmapProp, {
                pen: this.pen as Pen,
                loc: this.bitmapCursorPos,
            })
            if (data != null) {
    ...
    computed: {
        ...
        bitmapCursorPos(): Point {
            return {
                x: Math.floor(this.cursorPos.x / this.zoom),
                y: Math.floor(this.cursorPos.y / this.zoom),
            }
        },
        bitmapProp: {
            get(): Bitmap {
                return this.bitmap as Bitmap
            },
            set(v: Bitmap) {
                this.$emit('update:bitmap', v)
            },
        },
    },
    watch: {
